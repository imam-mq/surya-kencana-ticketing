...
const Btransfer = () => {
  ...
  const fetchTransferData = async () => {
    ...
    try {
      ...
      toast({ title: "Gagal memuat data", status: "error", duration: 3000, isClosable: true });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Flex minH="100vh" bg="gray.50">
        ...
        <Box p={8}>
            ...
            <CardBody>
              <Flex gap={4} align="center" wrap="wrap">
                ...
                <Flex align="center" gap={3} flex={1} minW="200px">
                  ...
                  <Input type="text" placeholder="Cari nama agent..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && fetchTransferData()} />
                </Flex>
          ...
          <TableContainer bg="white" borderRadius="xl" border="1px solid" borderColor="gray.200" shadow="sm">
            <Table variant="simple">
              <Thead bg="gray.100">
                <Tr><Th>No</Th><Th>Periode</Th><Th>Agent</Th><Th isNumeric>Tagihan</Th><Th isNumeric>Setor</Th><Th textAlign="center">Bukti</Th><Th textAlign="center">Status</Th><Th textAlign="center">Aksi</Th></Tr>
              </Thead>
              <Tbody>
                {loading ? (
                  <Tr><Td colSpan={8} textAlign="center" py={10}><Spinner size="md" color="blue.500" mr={3} /> Memuat data...</Td></Tr>
                  ...
                  transferData.map((item, index) => {
                    ...
                    return (
                      <Tr key={item.id} _hover={{ bg: "blue.50" }} transition="all 0.2s">
                        ...
                        <Td textAlign="center"><Badge as="button" onClick={() => window.open(`http://127.0.0.1:8000${item.bukti_transfer}`, "_blank")} colorScheme="orange" variant="outline" px={3} py={1} borderRadius="full" cursor="pointer">VIEW FILE</Badge></Td>
                        <Td textAlign="center"><Badge colorScheme={style.colorScheme} px={3} py={1} borderRadius="full">{style.label}</Badge></Td>
                        <Td textAlign="center">
                          {item.status === 'MENUNGGU' && (<Button colorScheme="blue" size="sm" onClick={() => handleOpenValidasi(item)} boxShadow="sm">Validasi</Button>)}
                        </Td>
                      </Tr>
                    );
                  })
      ...
      <ModalValidasiTransfer open={openModal} onClose={() => { setOpenModal(false); fetchTransferData(); }} data={selectedItem} />
    </Flex>
  );
};
