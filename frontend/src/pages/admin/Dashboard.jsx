...
const StatCard = ({ title, value, icon: Icon, iconClass, bgClass }) => {
    return (
        <div className={`p-5 rounded-xl shadow-lg transition duration-300 transform hover:scale-[1.02] ${bgClass} border border-gray-200`}>
            <div className="flex items-start justify-between">
                <div>
                    <p className="text-sm font-medium text-gray-700">{title}</p>
                    <p className="text-3xl font-bold mt-1 text-gray-900">{value}</p>
                </div>
                <div className={`p-3 rounded-xl ${iconClass}`} style={{ backgroundColor: 'white', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.08)' }}>
